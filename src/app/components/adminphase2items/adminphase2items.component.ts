...
@Component({
  selector: 'app-adminphase2items',
  templateUrl: './adminphase2items.component.html',
  styleUrls: ['./adminphase2items.component.css']
})
export class Adminphase2itemsComponent implements OnInit {
...
displayedRows$: Observable<any>;
    ...
    this.http2.get(this.rootURL).subscribe(
      res => {
        ...
        for(var j = 0; j < vals.length; j++){
          ...
          if(searcheddata.data.Items.includes('Phase')){
            this.treestruct = [];
  ...
  save(){
    ...
    for(var j = 0; j< this.treestructpass.length; j++){
      ...
      if(popdata.data.Items.includes('Phase')){
